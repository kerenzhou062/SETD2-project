...
library("ggsignif")
setwd("/data/zhoukr/hhl_setd2_m6a/analysis/joint-analysis/Hela/fpkm")

#peak fold enrichment
myData <- read.table(file="Hela_join_exp-histone-m6A_foldEnrichment_log2.txt",header=TRUE)
...
pdf("Hela_join_exp-histone-m6A_foldEnrichment_log2_H3K36me3.pdf", height=8)
plot(boxplot)
dev.off()
...
pdf("Hela_join_exp-histone-m6A_foldEnrichment_log2_m6A.pdf", height=8)
plot(boxplot)
dev.off()

#gene fpkm
myData <- read.table(file="Hela_join_exp-histone-m6A_fpkm_log2.txt",header=TRUE)
myData$xorder <- factor(myData$expClass, levels = c("low","medium","high"))
ylim1 <- boxplot.stats(myData$H3K36me3)$stats[c(1, 5)]
tipLen = c(0.01, 0.01)
vjust = 0.3
...
pdf("Hela_join_exp-histone-m6A_fpkm_log2_H3K36me3.pdf", height=8)
plot(boxplot)
dev.off()
...
boxplot <- ggplot(myData, aes(x = xorder, y = m6A, fill=xorder)) +
  ...
  geom_signif(comparisons = list(c("low", "medium")),
               map_signif_level = TRUE, test = "wilcox.test", textsize=5,
               y_position=ylim1[2]*1.5, tip_length = tipLen, vjust=vjust) +
  geom_signif(comparisons = list(c("medium", "high")),
               map_signif_level = TRUE, test = "wilcox.test", textsize=5,
               y_position=ylim1[2]*1.54, tip_length = tipLen, vjust=vjust) +
  geom_signif(comparisons = list(c("low", "high")),
               map_signif_level = TRUE, test = "wilcox.test", textsize=5,
               y_position=ylim1[2]*1.58, tip_length = tipLen, vjust=vjust) +
  coord_cartesian(ylim = c(ylim1[1]*0.9, ylim1[2]*1.6))

pdf("Hela_join_exp-histone-m6A_fpkm_log2_m6A.pdf", height=8)
plot(boxplot)
dev.off()
