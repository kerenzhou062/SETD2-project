...
library("ggsignif")
setwd("/data/zhoukr/hhl_setd2_m6a/analysis/joint-analysis/HepG2/DEGseq")

#gene fpkm
myData <- read.table(file="HepG2_join_exp-m6A_DEGseq_log2.txt",header=TRUE)
...
pdf("HepG2_join_exp-m6a-DEGseq_log2.pdf", height=8)
plot(boxplot)
dev.off()

myData <- read.table(file="HepG2_join_exp-histone_DEGseq_log2.txt",header=TRUE)
myData$Type <- factor(myData$expClass, levels = c("low","medium","high"))
...
pdf("HepG2_join_exp-histone-DEGseq_log2.pdf", height=8)
plot(boxplot)
dev.off()
