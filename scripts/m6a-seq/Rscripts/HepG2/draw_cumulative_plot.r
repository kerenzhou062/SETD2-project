...
setwd("/data/zhoukr/hhl_setd2_m6a/analysis/m6a-seq/HepG2/xls/overlappedPeak")

M14TargetFC <- read.table("./target/HepG2_shM14_FC_log2.txt",sep="\t",header = FALSE)
M3TargetFC <- read.table("./target/HepG2_shM3_FC_log2.txt",sep="\t",header = FALSE)
WTAPTargetFC <- read.table("./target/HepG2_shWTAP_FC_log2.txt",sep="\t",header = FALSE)
shareTargetFC <- read.table("./target/HepG2_shareTargets_FC_log2.txt",sep="\t",header = FALSE)
nonTargetFC <- read.table("./target/HepG2_nonTargets_FC_log2.txt",sep="\t",header = FALSE)
...
myData <- read.table("HepG2_cumulativePlot.txt",sep="\t",header = TRUE)
...
pdf("HepG2_overlappedCumulativePlot.pdf", height=5)
plot(overlappedPeakCum)
dev.off()

#### m6A and H3K36me3
setwd("/data/zhoukr/hhl_setd2_m6a/analysis/m6a-seq/HepG2/xls/m6a-H3K36me3")
# all m6A sites in shCont and shSetD2
shCont<- read.table("./HepG2_shCont_m6a.txt",sep="\t",header = FALSE)
shSetD2 <- read.table("./HepG2_shSetD2_m6a.txt",sep="\t",header = FALSE)
...
myData <- read.table("HepG2_m6A_all_cumulativePlot.txt",sep="\t",header = TRUE)
...
cumulativePlot <- ggplot(data=reshapeData, aes(x=FE, y=value, colour=variable)) + geom_line(size=1.5)+ labs(x="Fold Enrichment(log2)", y = "Cumulative frequency") +
  theme(panel.background = element_rect(fill = "white", colour = "black"), axis.title = element_text(size = 13, face = "bold"),
    ...
    legend.justification=c(0,1), legend.position=c(0.01,0.99), panel.grid.major.x = element_line(color = "grey80"),
        panel.grid.major.y = element_line(color = "grey80"))
pdf("HepG2_m6A_all_cumulativePlot.pdf", height=5)
plot(cumulativePlot)
dev.off()

## overlapped m6A sites in shCont and shSetD2 in H3K36me3
shCont<- read.table("./HepG2_intersect_shCont_m6a+H3K36me3.txt",sep="\t",header = FALSE)
shSetD2 <- read.table("./HepG2_intersect_shSetD2_m6a+H3K36me3.txt",sep="\t",header = FALSE)
wilcoxTest <- wilcox.test(shCont$V1,shSetD2$V1, alternative="greater", paired = FALSE, exact = TRUE, correct = TRUE)
wilcoxPVal = pValFunction(wilcoxTest$p.value)

myData <- read.table("HepG2_intersect_m6A_H3K36me3_cumulativePlot.txt",sep="\t",header = TRUE)
reshapeData <- melt(myData,id="FE")
levels(reshapeData$variable)[levels(reshapeData$variable)=="shCont"] <- "Control"
levels(reshapeData$variable)[levels(reshapeData$variable)=="shSetD2"] <- paste("shSETD2", " (p ", wilcoxPVal,")", sep = "")
cumulativePlot <- ggplot(data=reshapeData, aes(x=FE, y=value, colour=variable)) + geom_line(size=1.5)+ labs(x="Fold Enrichment (log2)", y = "Cumulative frequency") +
  theme(panel.background = element_rect(fill = "white", colour = "black"), axis.title = element_text(size = 13, face = "bold"),
    ...
    legend.justification=c(0,1), legend.position=c(0.01,0.99), panel.grid.major.x = element_line(color = "grey80"),
        panel.grid.major.y = element_line(color = "grey80"))
pdf("HepG2_intersect_m6A_H3K36me3_cumulativePlot.pdf", height=5)
plot(cumulativePlot)
dev.off()

myData <- read.table("HepG2_intersect_m6A_H3K36me3.txt",sep="\t",header = TRUE)
...
pdf("HepG2_intersect_m6A_H3K36me3_scatter.pdf")
plot(p1)
dev.off()

## overlapped m6A sites in shCont and shSetD2
shCont<- read.table("./HepG2_intersect_shCont_m6a+shSetD2.txt",sep="\t",header = FALSE)
shSetD2 <- read.table("./HepG2_intersect_shSetD2_m6a+shCont.txt",sep="\t",header = FALSE)
wilcoxTest <- wilcox.test(shCont$V1,shSetD2$V1, alternative="greater", paired = FALSE, exact = TRUE, correct = TRUE)
wilcoxPVal = pValFunction(wilcoxTest$p.value)

myData <- read.table("HepG2_intersect_m6A_cumulativePlot.txt",sep="\t",header = TRUE)
reshapeData <- melt(myData,id="FE")
levels(reshapeData$variable)[levels(reshapeData$variable)=="shCont"] <- "Control"
levels(reshapeData$variable)[levels(reshapeData$variable)=="shSetD2"] <- paste("shSETD2", " (p ", wilcoxPVal,")", sep = "")
cumulativePlot <- ggplot(data=reshapeData, aes(x=FE, y=value, colour=variable)) + geom_line(size=1.5)+ labs(x="Fold Enrichment (log2)", y = "Cumulative frequency") +
  theme(panel.background = element_rect(fill = "white", colour = "black"), axis.title = element_text(size = 13, face = "bold"),
    ...
    legend.justification=c(0,1), legend.position=c(0.01,0.99), panel.grid.major.x = element_line(color = "grey80"),
        panel.grid.major.y = element_line(color = "grey80"))
pdf("HepG2_intersect_m6A_cumulativePlot.pdf", height=5)
plot(cumulativePlot)
dev.off()

myData <- read.table("HepG2_intersect_m6A.txt",sep="\t",header = TRUE)
wilcoxTest <- wilcox.test(myData$shCont,myData$shSETD2, alternative="greater", paired = TRUE, exact = TRUE, correct = TRUE)
wilcoxPVal = pValFunction(wilcoxTest$p.value)
p1 <- ggplot(myData, aes(x=shCont, y=shSETD2)) +
  geom_point(colour="blue",size=0.8) + xlim(1, 8) + ylim(1,8) + geom_abline(slope=1, intercept=0, color='red', size=1) +
  ...
  theme(axis.title = element_text(size = 14, face = "bold"),
        axis.text = element_text(size = 13, face = "bold"))
pdf("HepG2_intersect_m6A_scatter.pdf")
plot(p1)
dev.off()
