...
#### m6A and H3K36me3
setwd("/data/zhoukr/hhl_setd2_m6a/analysis/m6a-seq/mESCs/xls/m6a-H3K36me3")
# all m6A sites in shCont and shSetD2
shCont<- read.table("./mESCs_shCont_m6a.txt",sep="\t",header = FALSE)
shSetD2 <- read.table("./mESCs_shSetD2_m6a.txt",sep="\t",header = FALSE)
...
myData <- read.table("mESCs_m6A_all_cumulativePlot.txt",sep="\t",header = TRUE)
...
pdf("mESCs_m6A_all_cumulativePlot.pdf", height=5)
plot(cumulativePlot)
dev.off()

## overlapped m6A sites in shCont and shSetD2 in H3K36me3
shCont<- read.table("./mESCs_intersect_shCont_m6a+H3K36me3.txt",sep="\t",header = FALSE)
shSetD2 <- read.table("./mESCs_intersect_shSetD2_m6a+H3K36me3.txt",sep="\t",header = FALSE)
wilcoxTest <- wilcox.test(shCont$V1,shSetD2$V1, alternative="greater", paired = FALSE, exact = TRUE, correct = TRUE)
wilcoxPVal = pValFunction(wilcoxTest$p.value)

myData <- read.table("mESCs_intersect_m6A_H3K36me3_cumulativePlot.txt",sep="\t",header = TRUE)
reshapeData <- melt(myData,id="FE")
levels(reshapeData$variable)[levels(reshapeData$variable)=="shCont"] <- "Control"
levels(reshapeData$variable)[levels(reshapeData$variable)=="shSetD2"] <- paste("shSETD2", " (p ", wilcoxPVal,")", sep = "")
cumulativePlot <- ggplot(data=reshapeData, aes(x=FE, y=value, colour=variable)) + geom_line(size=1.5)+ labs(x="Fold Enrichment (log2)", y = "Cumulative frequency") +
  theme(panel.background = element_rect(fill = "white", colour = "black"), axis.title = element_text(size = 13, face = "bold"),
    ...
    legend.justification=c(0,1), legend.position=c(0.01,0.99), panel.grid.major.x = element_line(color = "grey80"),
        panel.grid.major.y = element_line(color = "grey80"))
pdf("mESCs_intersect_m6A_H3K36me3_cumulativePlot.pdf", height=5)
plot(cumulativePlot)
dev.off()

myData <- read.table("mESCs_intersect_m6A_H3K36me3.txt",sep="\t",header = TRUE)
...
pdf("mESCs_intersect_m6A_H3K36me3_scatter.pdf")
plot(p1)
dev.off()
